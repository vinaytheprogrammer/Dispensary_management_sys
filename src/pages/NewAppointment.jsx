...
const NewAppointment = () => {
    ...
    const handleFetchDetails = async () => {
        try {
            const response = await axios.post("http://localhost:3000/api/staff/getinformation", {
                user_id: formData.id,
            });
    ...
    const handleFetchDependents = async () => {
        try {
            const response = await axios.get(`http://localhost:3000/api/staff/getdependents/${formData.id}`);
            if (response.status === 200) {
                ...
                {appointmentType && (
                    <>
                        ...
                        {appointmentType === 'dependent' && !detailsFetched && (
                            <div>
                                <button type="button" onClick={handleFetchDependents} className="w-full bg-blue-900 text-white p-2 rounded mb-4">Fetch Dependents</button>
                                {dependents.length > 0 && (
                                    <div className="mb-4">
                                        <label htmlFor="dependent" className="block text-lg mb-2">Select Dependent:</label>
                                        <select id="dependent" value={selectedDependent} onChange={handleDependentChange} className="w-full p-2 border border-gray-300 rounded">
                                ...
                                {selectedDependent === 'new' && (
                                    <button type="button" onClick={() => setNewDependent(true)} className="w-full bg-blue-900 text-white p-2 rounded mb-4">Add Dependent Details</button>
                                )}
                            </div>
                        )}
                                ...
                                {newDependent && (
                                    <div className="mt-4 border border-gray-300 p-4">
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <input type="text" id="name" name="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full p-2 border border-gray-300 rounded" required />
                                        </div>
                                        <div className="mb-4">
                                            <label htmlFor="dob" className="block text-lg mb-2">Date of Birth:</label>
                                            <input type="date" id="dob" name="dob" value={formData.dob} onChange={(e) => setFormData({ ...formData, dob: e.target.value })} className="w-full p-2 border border-gray-300 rounded" required />
                                        </div>
                                        <div className="mb-4">
                                            <label htmlFor="gender" className="block text-lg mb-2">Gender:</label>
                                            <select id="gender" name="gender" value={formData.gender} onChange={(e) => setFormData({ ...formData, gender: e.target.value })} className="w-full p-2 border border-gray-300 rounded" required>
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <input type="tel" id="contactNo" name="contactNo" value={formData.contactNo} onChange={(e) => setFormData({ ...formData, contactNo: e.target.value })} className="w-full p-2 border border-gray-300 rounded" required />
                                        </div>
                                    </div>
                                )}
                                <div className="mt-4 border border-gray-300 p-4">
                                    <h3 className="text-xl mb-2">Reason for Visit</h3>
                                    <textarea id="reasonForVisit" name="reasonForVisit" value={formData.reasonForVisit} onChange={(e) => setFormData({ ...formData, reasonForVisit: e.target.value })} className="w-full p-2 border border-gray-300 rounded" required />
                                </div>
                                <div className="mt-4 border border-gray-300 p-4">
                                    ...
                                    {editVitals ? (
                                        <div>
                                            ...
                                            <button type="button" onClick={handleSaveChanges} className="w-full bg-blue-900 text-white p-2 rounded">Save Changes</button>
                                        </div>
                                    ) : (
                                        <div>
                                            <p><strong>Height:</strong> {formData.height}</p>
                                            <p><strong>Weight:</strong> {formData.weight}</p>
                                            <p><strong>Body Temperature:</strong> {formData.bodyTemperature}</p>
                                            <p><strong>Blood Pressure:</strong> {formData.bloodPressure}</p>
                                            <p><strong>Blood Group:</strong> {formData.bloodGroup}</p>
                                            <p><strong>SpO2:</strong> {formData.spo2}</p>
                                            <button type="button" onClick={handleUpdateVitals} className="w-full bg-blue-900 text-white p-2 rounded">Update Vitals</button>
                                        </div>
                                    )}
                                </div>
                                <button type="submit" className="w-full bg-green-500 text-white p-2 rounded mt-4">Submit</button>
                            </>
                        )}
                    </>
                )}
